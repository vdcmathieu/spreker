...
Live at **[lab.vandecatsije.com/spreker](https://lab.vandecatsije.com/spreker)**.

It is its own Vercel project, `spreker`, serving from `https://spreker.vercel.app` with `basePath` `/spreker`, and the lab host proxies it - exactly like `yomu` and `mouvementdargent`.
The proxy is the branch on `/spreker` in `middleware.ts` in the `vandecatsije.com` repo, and the lab card is `src/content/lab/spreker.md` there.
...
The project is not connected to the GitHub repo, so a push does not deploy on its own; the CLI does.
`NEXT_PUBLIC_BASE_PATH` is set to `/spreker` on production only, which is why `pnpm dev` still serves from the root.
`NEXT_PUBLIC_SITE_URL` is unset and defaults to `https://lab.vandecatsije.com`.
